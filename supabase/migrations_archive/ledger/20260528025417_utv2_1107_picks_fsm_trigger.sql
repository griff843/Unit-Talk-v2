...
COMMENT ON FUNCTION public.picks_fsm_transition_guard() IS
  'Enforces the canonical pick lifecycle FSM for every picks.status update, including service-role direct UPDATEs that bypass the TypeScript lifecycle layer.';
...
COMMENT ON TRIGGER picks_fsm_guard ON public.picks IS
  'DB-level FSM guard (UTV2-1107). Fires only when status changes. Raises SQLSTATE P0001 / FSM_PICK_TRANSITION_REJECTED on invalid transitions.';;
