-- UTV2-871
-- Purpose: keep bounded provider_offers retention inside statement_timeout
-- after the provider_offers cutover made public.provider_offers a view over the
-- large quarantine table.
--
-- Fixes:
--   1. add the missing created_at/id index on provider_offers_legacy_quarantine
--      without blocking live reads/writes
--   2. retarget prune_provider_offers_bounded() to the quarantine base table
--      so the bounded delete runs directly against the indexed storage table

CREATE INDEX CONCURRENTLY IF NOT EXISTS provider_offers_legacy_quarantine_created_at_id_idx
  ON public.provider_offers_legacy_quarantine (created_at ASC, id ASC);
