...
SET payload = sr.payload || jsonb_build_object(
  ...
  CASE
    ...
    WHEN sr.result = 'win' AND p.odds IS NOT NULL AND p.odds < 0
      THEN ROUND(COALESCE(p.stake_units, 1.0) * (100.0 / ABS(p.odds::numeric)), 2)
    WHEN sr.result = 'win'
