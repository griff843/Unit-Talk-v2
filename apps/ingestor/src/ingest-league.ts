...
export async function ingestLeague(
  ...
  try {
    ...
    if (!options.skipResults) {
      const resultsFetch = await fetchSGOResults({
        apiKey,
        league,
        snapshotAt,
      ...
      // Entity-resolve completed events so they exist in the DB before inserting results.
      // Phase 1/2 only processes upcoming events (oddsAvailable=true); completed events
      // in the lookback window may never have gone through entity resolution.
      if (resultsFetch.resolvedEvents.length > 0) {
        await resolveSgoEntities(resultsFetch.resolvedEvents, repositories, {
          ...(options.logger ? { logger: options.logger } : {}),
        });
      }

      resolvedResults = await resolveAndInsertResults(
