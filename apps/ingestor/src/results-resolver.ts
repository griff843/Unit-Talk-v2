...
export async function resolveAndInsertResults(
  ...
  for (const eventResult of eventResults) {
    try {
      const found = await repositories.events.findByExternalId(eventResult.providerEventId);
      if (!found) {
        // Event not in DB at all — can't insert results without a canonical event_id
        summary.skippedResults += eventResult.playerStats.length;
        continue;
      }

      // If SGO says the event is complete but the DB still has it as scheduled/in_progress,
      // update the status now. fetchSGOResults already validated completed+finalized.
      const event =
        found.status !== 'completed' && found.external_id !== null
          ? await repositories.events.upsertByExternalId({
              externalId: found.external_id,
              sportId: found.sport_id,
              eventName: found.event_name,
              eventDate: found.event_date,
              status: 'completed',
