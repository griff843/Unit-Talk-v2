...
describe('UTV2-1109: Dual-Authorization Runtime — INIT-2.4.2', () => {
  it('DUAL_AUTH_ACTIONS contains all required governed actions', () => {
    assert.ok(DUAL_AUTH_ACTIONS.includes('picks:void'), 'picks:void required');
    assert.ok(DUAL_AUTH_ACTIONS.includes('picks:override'), 'picks:override required');
    assert.ok(DUAL_AUTH_ACTIONS.includes('member:write'), 'member:write required');
    assert.ok(DUAL_AUTH_ACTIONS.includes('operator:admin'), 'operator:admin required');
    assert.ok(DUAL_AUTH_ACTIONS.includes('settlement:correct'), 'settlement:correct required');
    assert.ok(DUAL_AUTH_ACTIONS.includes('promotion:override'), 'promotion:override required');
    assert.ok(DUAL_AUTH_ACTIONS.length >= 6, 'at least 6 dual-auth actions required');
  });

  it('DUAL_AUTH_TTL_SECONDS is 3600 (1 hour)', () => {
    assert.equal(DUAL_AUTH_TTL_SECONDS, 3600);
  });

  it('requiresDualAuth: returns true for governed actions', () => {
  ...
  it('requiresDualAuth: returns false for non-governed actions', () => {
  ...
  it('createPendingApproval: produces a frozen PendingApproval with correct TTL', () => {
    ...
    const expectedExpiry = new Date(new Date(NOW).getTime() + 3600 * 1000).toISOString();
    assert.equal(pending.expiresAt, expectedExpiry);
    assert.ok(Object.isFrozen(pending), 'PendingApproval must be frozen (immutable)');
  });

  it('createPendingApproval: respects custom TTL', () => {
    const pending = createPendingApproval({
      id: 'pa-002',
    ...
    const expectedExpiry = new Date(new Date(NOW).getTime() + 1800 * 1000).toISOString();
    assert.equal(pending.expiresAt, expectedExpiry);
  });

  it('completeApproval: succeeds with different operator', () => {
    const pending = createPendingApproval({
      id: 'pa-003',
    ...
    assert.ok(Object.isFrozen(record), 'ApprovalRecord must be frozen (immutable)');
  });

  it('ADVERSARIAL: same-operator second approval is rejected with DualAuthViolationError', () => {
    const pending = createPendingApproval({
      id: 'pa-004',
      action: 'picks:void',
      firstApproverId: 'alice',
    ...
    assert.ok(caught instanceof DualAuthViolationError, 'must throw DualAuthViolationError');
    assert.ok(caught.message.includes('DUAL_AUTH_VIOLATION'), 'error must contain DUAL_AUTH_VIOLATION');
    assert.ok(caught.message.includes('alice'), 'error must name the offending operator');
    assert.equal(caught.action, 'picks:void');
    assert.equal(caught.code, 'DUAL_AUTH_VIOLATION');
  });

  it('ADVERSARIAL: same-operator approval on operator:admin is rejected', () => {
    const pending = createPendingApproval({
      id: 'pa-005',
      action: 'operator:admin',
    ...
    assert.throws(
      () => completeApproval({ pending, secondApproverId: 'carol', approvedAt: LATER }),
      DualAuthViolationError,
    );
  });

  it('ADVERSARIAL: same-operator approval on member:write is rejected', () => {
    const pending = createPendingApproval({
      id: 'pa-006',
      action: 'member:write',
    ...
    assert.throws(
      () => completeApproval({ pending, secondApproverId: 'dave', approvedAt: LATER }),
      DualAuthViolationError,
    );
  });

  it('ADVERSARIAL: expired pending approval is rejected', () => {
    const pending = createPendingApproval({
      id: 'pa-007',
    ...
    let caught: unknown;
    try {
      completeApproval({ pending, secondApproverId: 'bob', approvedAt: AFTER_TTL });
    } catch (e) {
      caught = e;
    }
  ...
  it('ADVERSARIAL: expired pending approval on promotion:override is rejected', () => {
    const pending = createPendingApproval({
      id: 'pa-008',
    ...
    assert.throws(
      () => completeApproval({ pending, secondApproverId: 'frank', approvedAt: AFTER_TTL }),
      DualAuthViolationError,
    );
  });

  it('isDualAuthExpired: returns true when past TTL', () => {
    const pending = createPendingApproval({
      id: 'pa-009',
      action: 'picks:void',
      firstApproverId: 'alice',
  ...
  it('replayApprovalChain: deterministically reconstructs the ApprovalRecord', () => {
    const pending = createPendingApproval({
      id: 'pa-010',
      action: 'picks:override',
      firstApproverId: 'alice',
  ...
  it('ApprovalRecord is immutable — mutations are rejected in strict mode', () => {
    const pending = createPendingApproval({
      id: 'pa-011',
      action: 'operator:admin',
      firstApproverId: 'alice',
  ...
  it('DualAuthViolationError has correct name and code', () => {
