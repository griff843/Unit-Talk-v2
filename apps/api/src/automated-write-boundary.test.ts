...
function automatedBoardPayload(
...
export function registerAutomatedWriteBoundaryTests(
  register: RegisterTest,
): void {
  register(
    'automated write boundary materializes a board pick directly into awaiting_approval',
    async () => {
      const repositories = createInMemoryRepositoryBundle();
      const result = await processSubmission(
        automatedBoardPayload(),
        repositories,
      );
      ...
      assert.match(
        result.lifecycleEvent.reason,
        /scheduler:board-pick-writer requires operator approval/,
      );
  ...
  register(
    'automated write boundary preserves the human/manual validated path',
    async () => {
      const repositories = createInMemoryRepositoryBundle();
      const result = await processSubmission(
        {
  ...
  register(
    'automated write boundary rejects missing market evidence before persistence',
    () => {
  ...
  register(
    'automated write boundary rejects a stale source snapshot before persistence',
    () => {
      const payload = automatedBoardPayload({
        metadata: {
      ...
      assert.throws(
        ...
        (error: unknown) =>
          error instanceof AutomatedWriteBoundaryError &&
          error.code === 'STALE_PRICE_SNAPSHOT',
      );
    },
  );
