...
import type { CanonicalPick, PickSource, SubmissionPayload } from '@unit-talk/contracts';
import {
...
import { enqueueDistributionWork, isGovernanceBrakeSource } from './distribution-service.js';
import {
...
import {
  ...
  type SubmitPickControllerResult,
} from './controllers/submit-pick-controller.js';
import {
...
// ---------------------------------------------------------------------------
// UTV2-1611 automated write boundary
//
// These cases live here (rather than in a standalone module) because
// submission-service.ts is where the boundary is actually invoked, and this
// file is named directly by the `test:apps-api-core` package script — so the
// coverage is physically reachable from a wired root instead of depending on
...
test('a source-only automated submission is left to the Phase 7A source brake', async () => {
...
test('synthetic future system producer cannot bypass the governed boundary', async () => {
...
test('REGRESSION UTV2-1611: an unmarked board-construction production never persists as validated', async () => {
...
test('REGRESSION UTV2-1611: an unmarked board production without producer identity fails closed', async () => {
  const repositories = createInMemoryRepositoryBundle();
...
test('REGRESSION UTV2-1611: an unmarked board production without market evidence fails closed', () => {
  assert.throws(
    () =>
      prepareAutomatedSubmission(
...
test('REGRESSION UTV2-1611: board-construction carries a source-keyed fallback brake', () => {
...
test('REGRESSION UTV2-1611: the submit controller does not re-apply awaiting_approval after atomic materialization', async () => {
  const repositories = createInMemoryRepositoryBundle();
...
test('REGRESSION UTV2-1611: the source-only Phase 7A brake path is still governed by the controller', async () => {
...
test('REGRESSION UTV2-1611: the live proof cleans up through the lifecycle FSM, not a direct status PATCH', () => {
  // The live staging assertions for this lane live in the already-wired,
  // already-classified `t1-proof-awaiting-approval.test.ts`. Its fixture
  // cleanup must void through the FSM so every status change carries a
  // matching `pick_lifecycle` audit row.
  const proofPath = fileURLToPath(
    new URL('./t1-proof-awaiting-approval.test.ts', import.meta.url),
  );
  ...
  assert.match(
    source,
    /transitionPickLifecycle\(\s*\n?\s*repositories\.picks,/,
    'live proof cleanup must void fixtures through transitionPickLifecycle',
  );
  assert.match(
    source,
    /'voided',[\s\S]{0,200}?'operator_override'/,
    'the board-boundary fixture cleanup must void via operator_override through the FSM',
  );
  assert.match(
    source,
    /to_state=eq\.voided/,
    'the live proof must assert the voiding cleanup wrote its own pick_lifecycle audit row',
  );
});

test('REGRESSION UTV2-1611: the automated/braked drift invariant fails on the condition it names', () => {
...
test('REGRESSION UTV2-1611: validated is a legal transient for a marker-less Phase 7A brake source', () => {
