...
async function insert(table: string, row: Record<string, unknown>) {
  ...
  if (!resp.ok)
    throw new Error(`INSERT ${table} failed: ${JSON.stringify(body)}`);
...
for (const name of [
  ...
  test(
    `STEP 1 — RPC existence: ${name} exists`,
    { skip: skipReason },
    async () => {
      const resp = await fetch(`${SUPABASE_URL}/rest/v1/rpc/${name}`, {
        method: 'POST',
      ...
      const notFound =
        body.message?.includes('does not exist') || body.code === '42883';
...
test(
  'STEP 2 — Submission atomicity: A. atomic submission creates all records',
  { skip: skipReason },
  async () => {
    const now = new Date().toISOString();
    const { data, error } = await rpc<SubmissionAtomicResponse>(
      'process_submission_atomic',
      {
        p_submission: {
          id: submissionId,
          source: 't1-proof',
          submitted_by: 'proof-runner',
    ...
    console.log(
      `  ✓ Submission ${submissionId}, Pick ${pickId} created atomically`,
    );
  },
);

test(
  'STEP 2 — Submission atomicity: B. duplicate returns existing pick (idempotent)',
  { skip: skipReason },
  async () => {
    const now = new Date().toISOString();
    const { data, error } = await rpc<SubmissionAtomicResponse>(
    ...
    assert.equal(
      data.pick.id,
      pickId,
      `Idempotency FAILED: expected ${pickId}, got ${data.pick.id}`,
    );
...
test(
  'STEP 2 — Submission atomicity: C. exactly 1 pick for idempotency key',
  { skip: skipReason },
  async () => {
    const picks = await query<PickIdRow>(
      'picks',
      `idempotency_key=eq.${idempotencyKey}&select=id`,
    );
...
test(
  'STEP 3 — Enqueue atomicity: A. atomic enqueue transitions + creates outbox',
  { skip: skipReason },
  async () => {
    const { data, error } = await rpc<EnqueueAtomicResponse>(
      'enqueue_distribution_atomic',
      {
        p_pick_id: enqueuePickId,
        p_from_state: 'validated',
        p_to_state: 'queued',
        p_writer_role: 'promoter',
        p_reason: 'proof: ready',
...
test(
  'STEP 3 — Enqueue atomicity: B. no zombie pick (outbox exists for queued pick)',
  { skip: skipReason },
  async () => {
    const picks = await query<PickStatusRow>(
      'picks',
      `id=eq.${enqueuePickId}&select=status`,
    );
    assert.equal(picks[0]?.status, 'queued');

    const outbox = await query<OutboxRow>(
      'distribution_outbox',
      `pick_id=eq.${enqueuePickId}&select=id,status`,
    );
...
test(
  'STEP 3 — Enqueue atomicity: C. re-enqueue returns null (already queued)',
  { skip: skipReason },
  async () => {
...
before(async () => {
  ...
  await insert('distribution_outbox', {
    ...
    status: 'processing',
    claimed_at: now,
    claimed_by: 'proof-worker',
    payload: {},
    idempotency_key: `t1-dlv-${dlvPickId}`,
  });
});

test(
  'STEP 4 — Delivery idempotency: A. claim returns null for empty target',
  { skip: skipReason },
  async () => {
    const { data, error } = await rpc<null>('claim_next_outbox', {
      p_target: `none-${randomUUID()}`,
      p_worker_id: 'proof',
    });
...
test(
  'STEP 4 — Delivery idempotency: B. confirm_delivery_atomic marks sent + lifecycle + receipt',
  { skip: skipReason },
  async () => {
    const { data, error } = await rpc<ConfirmDeliveryAtomicResponse>(
      'confirm_delivery_atomic',
      {
        p_outbox_id: dlvOutboxId,
        p_pick_id: dlvPickId,
        p_worker_id: 'proof-worker',
        p_receipt_type: 'discord.message',
        p_receipt_status: 'sent',
        p_receipt_channel: 'discord:#canary',
        p_receipt_external_id: 'msg-proof-123',
        p_receipt_idempotency_key: `t1-rcpt-${dlvOutboxId}`,
        p_receipt_payload: { proof: true },
        p_lifecycle_from_state: 'queued',
        p_lifecycle_to_state: 'posted',
        p_lifecycle_writer_role: 'poster',
        p_lifecycle_reason: 'delivery confirmed',
        p_audit_action: 'distribution.sent',
    ...
    console.log(
      `  ✓ Outbox=sent, lifecycle=posted, receipt created, audit logged`,
    );
  },
);

test(
  'STEP 4 — Delivery idempotency: C. re-confirm returns alreadyConfirmed=true (NO double post)',
  { skip: skipReason },
  async () => {
    const { data, error } = await rpc<ConfirmDeliveryAtomicResponse>(
      'confirm_delivery_atomic',
      {
        ...
        p_receipt_status: 'sent',
        p_receipt_channel: 'discord:#canary',
        p_receipt_external_id: 'msg-proof-456',
        p_receipt_idempotency_key: `t1-rcpt-retry-${dlvOutboxId}`,
        p_receipt_payload: {},
        p_lifecycle_from_state: 'queued',
        p_lifecycle_to_state: 'posted',
        p_lifecycle_writer_role: 'poster',
        p_lifecycle_reason: 'retry',
        p_audit_action: 'distribution.sent',
        p_audit_payload: {},
      },
    );
...
test(
  'STEP 4 — Delivery idempotency: D. pick is posted (not stuck in queued)',
  { skip: skipReason },
  async () => {
    const picks = await query<PickPostedRow>(
      'picks',
      `id=eq.${dlvPickId}&select=status,posted_at`,
    );
...
test(
  'STEP 5 — Settlement atomicity: A. atomic settlement creates record + transitions',
  { skip: skipReason },
  async () => {
    const { data, error } = await rpc<SettlePickAtomicResponse>(
      'settle_pick_atomic',
      {
        ...
        p_settlement: {
          ...
          settled_by: 'proof-runner',
          evidence_ref: 'proof-test',
          payload: { proof: true },
          settled_at: new Date().toISOString(),
        },
        p_lifecycle_from_state: 'posted',
        p_lifecycle_to_state: 'settled',
        p_lifecycle_writer_role: 'settler',
        p_lifecycle_reason: 'proof settlement',
        p_audit_action: 'settlement.recorded',
        p_audit_actor: 'proof-runner',
        p_audit_payload: { proof: true },
      },
    );
...
test(
  'STEP 5 — Settlement atomicity: B. duplicate returns existing (idempotent)',
  { skip: skipReason },
  async () => {
    const { data, error } = await rpc<SettlePickAtomicResponse>(
      'settle_pick_atomic',
...
test(
  'STEP 5 — Settlement atomicity: C. exactly 1 settlement record',
  { skip: skipReason },
  async () => {
    const settlements = await query<PickIdRow>(
      'settlement_records',
    ...
    assert.equal(
      settlements.length,
      1,
      `Expected 1, got ${settlements.length}`,
    );
...
test(
  'STEP 5 — Settlement atomicity: D. pick is settled with timestamp',
  { skip: skipReason },
  async () => {
    const picks = await query<PickSettledRow>(
      'picks',
      `id=eq.${settlePickId}&select=status,settled_at`,
    );
...
test('STEP 6 — Fail-closed: API refuses to start in fail_closed without DB', async () => {
  ...
  console.log(
    `  ✓ fail_closed rejects startup without DB — no silent data loss`,
  );
});
