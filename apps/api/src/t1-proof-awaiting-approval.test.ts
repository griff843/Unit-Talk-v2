...
async function runBrakeCase(source: PickSource) {
  ...
  assert.equal(
    response.status,
    201,
    `brake ${source}: expected 201, got ${response.status}`,
  );
  ...
  assert.equal(
    data.lifecycleState,
    'awaiting_approval',
    `brake ${source}: lifecycleState`,
  );
  assert.equal(
    data.governanceBrake,
    true,
    `brake ${source}: governanceBrake flag`,
  );
  ...
  const brakeEvents = lifecycleRows.filter(
    (row) =>
      row.from_state === 'validated' && row.to_state === 'awaiting_approval',
  );
  assert.equal(
    brakeEvents.length,
    1,
  ...
  assert.ok(
    brakeEvent.writer_role,
    `brake ${source}: writer_role must be non-empty`,
  );
  assert.ok(
    brakeEvent.reason && brakeEvent.reason.length > 0,
    `brake ${source}: reason must be non-empty`,
  );
...
test(
  'UTV2-519 brake path: system-pick-scanner',
  { skip: skipReason },
  async () => {
...
test(
  'UTV2-519 atomic rollback: mismatched fromState leaves picks.status and pick_lifecycle untouched',
  { skip: skipReason },
  async () => {
    ...
    const pickId = (resp.body as { ok: true; data: { pickId: string } }).data
      .pickId;
    createdPickIds.push(pickId);
    ...
    console.log(
      `  atomic rollback OK — pickId=${pickId} (cleaned up to voided)`,
    );
  },
);
...
test('UTV2-519 created pick ids (diagnostics)', { skip: skipReason }, () => {
  console.log(
    `  UTV2-519 test run created pick ids: ${JSON.stringify(createdPickIds)}`,
  );
});
