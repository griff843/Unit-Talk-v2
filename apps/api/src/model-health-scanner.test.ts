...
import { checkSchemaDrift, runModelHealthScan } from './model-health-scanner.js';
...
test('runModelHealthScan returns zero counts when no champion models exist', async () => {
  const deps = makeDeps();
...
test('runModelHealthScan scans a healthy champion model and writes a snapshot', async () => {
  const deps = makeDeps();
...
test('runModelHealthScan does not fire alert when model is green (roi > 0, no drift)', async () => {
  const deps = makeDeps();
...
test('runModelHealthScan fires warning alert when roi drops below -5%', async () => {
  const deps = makeDeps();
  const champion = await deps.modelRegistry.create({
    modelName: 'nhl-ml-v1',
    version: '1.0',
...
test('runModelHealthScan fires critical alert when roi drops below -15%', async () => {
  const deps = makeDeps();
  const champion = await deps.modelRegistry.create({
...
test('runModelHealthScan sets requiresOperatorDecision when critical window is exceeded', async () => {
  const deps = makeDeps();
  const champion = await deps.modelRegistry.create({
...
test('runModelHealthScan explicit slices override default sport scanning', async () => {
  const deps = makeDeps();
  await deps.modelRegistry.create({
...
test('runModelHealthScan skips non-champion models', async () => {
  const deps = makeDeps();
  await deps.modelRegistry.create({
...
test('runModelHealthScan scans multiple champion models across sports', async () => {
  const deps = makeDeps();
  await deps.modelRegistry.create({
    modelName: 'nba-spread-v1',
    version: '1.0',
...
test('checkSchemaDrift reports unreachable canonical tables and blocks materialization', async () => {
  const result = await checkSchemaDrift({
    canonicalTableNames: ['picks', 'pick_lifecycle'],
    logger: silentLogger(),
    probeTableCount: async (table) =>
      table === 'pick_lifecycle'
        ? {
            ...
            error: {
              code: 'PGRST204',
              message: "Could not find the 'pick_lifecycle' relation in the schema cache",
            },
          }
        : { count: 42, error: null },
  });
  ...
  assert.equal(result.materializationStatus, 'blocked');
  assert.equal(result.unreachableTables, 1);
  assert.deepEqual(result.unreachableTableNames, ['pick_lifecycle']);
  assert.equal(result.tables[0]?.status, 'ok');
  assert.equal(result.tables[1]?.status, 'unreachable');
  assert.match(result.remediation, /Reload the PostgREST schema cache/i);
});

test('evaluateModelHealthState requiresOperatorDecision when critical state exceeds window threshold', () => {
  const report = buildCriticalProxyReport(-20, 0.2, 0);
  const fortyEightHoursAgo = new Date(Date.now() - 48 * 60 * 60 * 1000).toISOString();

  const { newState, trigger } = evaluateModelHealthState(report, 'critical', 24, fortyEightHoursAgo);
...
test('evaluateModelHealthState leaves requiresOperatorDecision false within the critical window', () => {
  const report = buildCriticalProxyReport(-20, 0.2, 0);
  const twoHoursAgo = new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString();

  const { newState, trigger } = evaluateModelHealthState(report, 'critical', 24, twoHoursAgo);
