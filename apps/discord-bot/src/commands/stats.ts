...
export function buildStatsEmbed(stats: CapperStatsResponse) {
  const title =
    stats.scope === 'capper'
      ...
      : `📊 Server Stats — Last ${stats.window} Days${stats.sport ? ` (${stats.sport})` : ''}`;
  ...
  if (stats.picks === 0) {
    ...
    embed.setFooter({
      text: `Settled picks only · ${stats.picks} total · /pick to add a new pick`,
    });
    return embed;
  }
  ...
  const footerParts = [`Settled picks only · ${stats.picks} total · /pick to add a new pick`];
  if (stats.picks < 5) {
    footerParts.push('Insufficient sample for CLV stats.');
  }
...
export function createDefaultCommand(): CommandHandler {
  ...
  return createStatsCommand(createApiClient(config.operatorWebUrl));
}
