 ...
 *
 * Rewritten from `src/app/command-center-rebuild.test.tsx`, whose assertions
 * named copy from a much earlier revision of the pages ("Provider matrix",
 * "Model economics at a glance") and had gone stale unnoticed because that
 * file is not reachable from any package script. It stays unreachable — it is
 * a reviewed entry in `docs/05_operations/executable-wiring-baseline.json`,
 * outside this lane's file scope — so the rewritten assertions live here, in
 * a file the `test` script actually runs.
 */
