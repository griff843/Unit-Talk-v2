...
export default async function PromotionPreviewPage({ searchParams }: PromotionPreviewPageProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!pickId ? (
        <EmptyState message="Enter a pick ID" detail="Promotion preview dry-runs the live promotion engine without writes." />
      ) : preview?.ok ? (
        <section className="space-y-3">
          <div className="grid gap-3 md:grid-cols-3">
            <PreviewField label="Would promote to" value={preview.data.wouldPromoteTo ?? 'None'} />
            <PreviewField label="Score" value={preview.data.score === null ? 'None' : preview.data.score.toFixed(2)} />
            <PreviewField label="Qualification" value={preview.data.qualifies ? 'Qualified' : 'Not qualified'} />
          </div>
          <div className="rounded border border-gray-800 bg-gray-900/40 p-4">
            <p className="text-xs uppercase tracking-widest text-gray-500">Reasons</p>
            <ul className="mt-3 space-y-2">
              {preview.data.reasons.map((reason) => (
                <li key={reason} className="text-sm text-gray-200">{reason}</li>
              ))}
            </ul>
          </div>
        </section>
        ...
        <div className="rounded border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-200">
          {preview?.error ?? 'Promotion preview failed'}
        </div>
      )}
    </div>
  );
