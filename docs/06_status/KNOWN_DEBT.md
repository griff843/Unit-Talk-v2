...
| DEBT-025 | operational | `.lane/lanes/governance.yml`'s `allowed_path_globs` has never included `docs/06_status/KNOWN_DEBT.md`, so "Lane authority" fails on every governance-lane PR that edits it (reproduces on PR #1187 and PR #1191). Not fixed here to avoid widening this PR's declared file scope mid-flight for an unrelated gap. | Low | none — no Linear issue yet; open when `.lane/lanes/governance.yml` is next touched | `.lane/lanes/governance.yml` `allowed_path_globs` | open |
