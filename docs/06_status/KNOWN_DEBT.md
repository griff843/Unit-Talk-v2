...
| DEBT-024 | operational | `scripts/ops/pr-review-packet.ts`'s `sameIssueLaneMetadataPaths()` duplicates `scripts/ci/file-scope-guard.ts`'s `ownLaneControlPlanePatterns()` but is missing the `docs/06_status/proof/${issueId}/.gitkeep` entry, so "Return review packet" spuriously flags that file as out-of-scope for every fresh lane (reproduces on PR #1187 and others). Not fixed here — `pr-review-packet.ts` is outside this PR's declared file scope and widening it would require another lane-manifest reorg for an unrelated pre-existing gap. Note (UTV2-1518 reopened, 2026-07-13): `file-scope-guard.ts`'s own copy of this exemption was widened from `.gitkeep` to the full `docs/06_status/proof/${issueId}/**` glob — `pr-review-packet.ts`'s separate copy still only has `.gitkeep` and remains open here. | Low | none — no Linear issue yet; open when `pr-review-packet.ts` is next touched | `scripts/ops/pr-review-packet.ts` `sameIssueLaneMetadataPaths()` vs `scripts/ci/file-scope-guard.ts` `ownLaneControlPlanePatterns()` | open |
| DEBT-029 | operational | `scripts/ops/proof-auditor-gate.ts` flags "Required executed command lacks node:test pass evidence: pnpm test:db" even when a T3 lane's `verification.md` explicitly states `test:db` is NOT required/claimed for that tier — the checker appears to substring-match on the command name rather than parse negation/context, so any mention of `pnpm test:db` in prose is read as a claim of execution. Reproduced live on UTV2-1428 (PR #1196); non-blocking (Proof Auditor Gate is not in the repo's required-status-checks list). Not fixed here — recorded as post-lock repair debt per PM decision (2026-07-13) to keep the OS v1 lock path narrow. | Low | none — no Linear issue yet; open for post-lock repair | `scripts/ops/proof-auditor-gate.ts` | open |
| DEBT-030 | operational | `scripts/ops/shared.ts`'s `validateFileScopeEntry()` rejects any `file_scope_lock`/`expected_proof_paths` entry containing `*?[]{}` characters ("Glob patterns are not allowed in file scope") — but `scripts/ci/file-scope-guard.ts`'s `matchesLockPattern()` explicitly supports `/**` glob suffixes, and lane manifests routinely use them (e.g. `apps/command-center/**`, confirmed live on UTV2-1522's manifest). The same character-class regex also flags literal Next.js dynamic-route filenames like `picks/[id]/page.tsx` as "glob patterns" even though `[`/`]` there are just filesystem characters, not a glob. Net effect: `ops:lane-manifest record-merge` and `ops:lane-close --repair-merged` both hard-reject any manifest whose scope already contains a glob or a bracketed route path, forcing a manual JSON hand-edit to close the lane (reproduced live on UTV2-1522/PR #1190 post-merge closeout, 2026-07-13). Not fixed here — recorded for post-lock repair; the two validators (file-scope-guard's matcher vs shared.ts's entry validator) need to agree on what's a "glob" before either is authoritative. | Medium | none — no Linear issue yet; open for post-lock repair | `scripts/ops/shared.ts` `validateFileScopeEntry()` vs `scripts/ci/file-scope-guard.ts` `matchesLockPattern()` | open |
| DEBT-031 | operational | "Parked" lane manifests (relocated to `docs/06_status/lanes/parked/` per the UTV2-1519 mechanism, to drop out of `readAllManifestPaths()`'s non-recursive scan for concurrency-cap purposes) are still picked up by `scripts/ci/file-scope-guard.ts`'s git-based trusted-manifest reader (`createGitManifestSource`'s `git ls-tree -r`, which IS recursive) and still count as active for cross-lane file-scope conflict detection. Reproduced live: UTV2-1522/PR #1190 (superseding the parked UTV2-1512 foundation branch) got a spurious "Files locked by other active lanes" File-scope-lock failure against `docs/06_status/lanes/parked/UTV2-1512.json`. Non-blocking (File scope lock is not in the required-status-checks list) and scope-override doesn't apply (overrides only cover own-scope violations, not cross-lane conflicts). Not fixed here — recommend excluding `docs/06_status/lanes/parked/**` from file-scope-guard's manifest scan. | Low | none — no Linear issue yet; open for post-lock repair | `scripts/ci/file-scope-guard.ts` `createGitManifestSource().listPathsAtRef()` | open |

Current max ID: `DEBT-031`. Next insertion uses `DEBT-032`.
...
|---|---|---|---|---|
| DEBT-028 | `ownLaneControlPlanePatterns()` only exempted `docs/06_status/proof/<issue>/.gitkeep`, not the full proof directory (UTV2-1518 reopened) | 2026-07-13 | UTV2-1518 | this issue | UTV2-1518's original AC (auto-permit a lane's own canonical proof/manifest/sync paths without requiring `file_scope_lock` enumeration) was never actually implemented — the PR that closed it (#1184) fixed a different, real bug (scope_override branch-ownership check) instead. Confirmed live on UTV2-1428/PR #1196: a normal fresh multi-commit T3 docs lane whose `expected_proof_paths` gets populated in a commit AFTER the manifest's first-committed-content snapshot (the standard `ops:lane-start` → work → `ops:proof-generate` flow) fails File scope lock on its own proof files. Fixed: widened `ownLaneControlPlanePatterns()`'s proof-directory entry from the single `.gitkeep` file to the full `docs/06_status/proof/<issue-id>/**` glob, so the lane's own proof bookkeeping is always permitted regardless of expected_proof_paths trust-snapshot timing. Arbitrary file-scope widening beyond these three canonical paths is unaffected and still requires an externally authorized scope-override/v1 comment. 2 new regression tests reproduce the exact two-commit failure and confirm out-of-directory files still fail closed. |
