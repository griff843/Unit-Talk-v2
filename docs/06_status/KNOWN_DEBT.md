...
| DEBT-008 | schema | Promotion score components (`edge_score`, `trust_score`, `readiness_score`, `uniqueness_score`, `boardFit_score`) live inside `pick_promotion_history.payload` JSON — not as top-level columns. Queries that assume top-level columns fail. A view or typed accessor is the mitigation | Medium | none — open one before next intelligence-layer consumer touch | `docs/05_operations/CC_INTELLIGENCE_METRICS_REGISTER.md` | open |
