...
| File | Change |
|---|---|
| `scripts/ops/shared.ts` | `defaultProofPaths` declares `diff-summary.md`, `evidence.json` and `verification.md` for every tier, the set closeout P11/P3 and `ops:proof-generate` already demand. Only lane-start's declaration changes; manifests on disk keep their lists. |
...
| `scripts/ops/shared.test.ts` | Default set for T1/T2/T3; an explicitly declared list is kept verbatim. |
| `scripts/ops/truth-check-lib.test.ts` | The default set passes P11 at every tier by filename alone. |
...
which needs the execution SHA. T3 lanes now declare a bundle, so they are held to it.
