...
- [x] Harness: **88 passed, 0 failed** — all 28 pre-existing cases still green.
- [x] The tripwire's own escape hatches are closed. Each entry below records what the control did, what replaced it, and where an entry describes a revision that was itself later superseded, it says so rather than reading as current:
  - The opt-out test grepped the message for `plan-only` / `partial-fix` / `No-close:`. Any commit whose **prose** mentioned them silently disabled the tripwire — a fail-open inside the fix for a fail-open, reproduced on this lane's own squash merge. It was first changed to ask whether the close-intent **forms** matched and were filtered, which prose cannot fake; that was itself too loose and was narrowed again — see the entries below — so silencing now requires an explicit `No-close:` naming the subject's own closeout id.
...
- [x] Further holes found by later review rounds, each reproduced before fixing:
  ...
  - **An unrelated issue id in the body made the tripwire unreachable, and closed the wrong issue.** The workflow only consulted the tripwire when the extracted id set was empty; that set is built from the whole message while the signature is read from the subject. A drifted closeout whose body merely said `Closes <other>` therefore closed `<other>` and stranded its own lane in silence — no opt-out required, so strictly more reachable than the hatch below. The question was never "did we extract anything" but "did we extract the id this subject says it is closing"; `should_fire_closeout_tripwire()` now asks that.
  - **Requiring an issue id in the trailer-phrase signature was itself a regression.** It was added to stop a subject merely quoting the template from firing, but it made the most dangerous drift invisible: a template that stops emitting the id strands every lane at once, and requiring an id assumes exactly what that drift breaks. The quotation case was then excluded by detecting quote characters, which was wrong in both directions and is recorded below; the discriminator that survives is the `chore(` scope anchor.
  ...
  - **Reverts were under-covered and the phrase match was unanchored.** `Reapply "..."` — the subject `git revert` writes when reverting a revert — plus `revert!:`, `Reverts:` and the conventional-commit scoped forms `revert(lanes):` / `reapply(ci):` all reddened `main`, and a subject merely *quoting* the closeout template did too. Revert and reapply prefixes are now matched by a regex over the leading token, covering scopes and inflections, because enumerating literal prefixes missed cases twice, and the trailer-phrase signature is anchored to the producer's `chore(` scope rather than to a guess about punctuation. Two earlier discriminators — requiring an issue id, then excluding subjects containing a quote character — were each wrong in **both** directions: the first silenced the id-dropping drift, the second silenced any drift whose subject held an apostrophe while firing on documentation that quotes the phrase with backticks or asterisks. `chore(` is a structural property of the producer, stable across every drift this signature exists to catch and absent from the docs commits that were reddening `main`.
...
- [x] `scripts/ci/r-level-check.ts` evaluated: verdict PASS, 7 changed files, no R-level artifacts required for this diff.
