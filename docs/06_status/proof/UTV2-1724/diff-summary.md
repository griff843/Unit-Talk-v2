...
2. **`has_lane_closeout_signature()` — subject line only.** Recognises a commit produced by the closeout path: its conventional-commit scope with any close verb form, or the literal trailer phrase, both read from the **subject**. Deliberately **broader** than the grammar that consumes it, so template drift keeps matching the signature while the grammar stops. It does *not* require an issue id — a template that drops the id is the drift most likely to strand every lane at once. A subject merely *quoting* the phrase is excluded by detecting the quoting. Revert and reapply subjects are exempt, matched by a regex over the leading token so scopes (`revert(lanes):`) and inflections (`Reverts:`) are covered.
