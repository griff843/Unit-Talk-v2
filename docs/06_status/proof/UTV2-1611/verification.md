...
- [x] **`pnpm test:db` runs against the approved staging project on every pushed head, including this one, as part of the required `verify` context.** Verified at this head: CI run 32380078496, job 96460836194, `head_sha` equal to this head, conclusion `success`, with both "Run writable DB proof against staging" and "Run the T1 live proof suites against staging" succeeding. Every head this lane has pushed shows the same job green.
- [x] **`verify` enforces a same-run receipt, not this bundle.** `ci.yml` downloads `utv2-1630-db-proof-receipt-${{ github.run_id }}-${{ github.run_attempt }}` — scoped to the run that just produced it — and checks it with `scripts/ci/verify-db-proof-receipt.ts`. It never reads `evidence.json`. So the staging guarantee for this head comes from CI at this head, and a stale bundle could not weaken it.
...
`pnpm test:db` executed against the approved staging project and passed. It runs on **every** pushed head of this lane, and every one is green — including this head (run 32380078496, job 96460836194).
