...
- [x] The branch is current with `origin/main` — `git rev-list --left-right --count origin/main...HEAD` reports **0 behind / 37 ahead**. The head is a merge commit from the sanctioned `pr-update-branch` refresh, not a rebase. Every commit references only this issue.
