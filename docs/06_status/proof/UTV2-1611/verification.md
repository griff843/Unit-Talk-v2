...
- [x] The staging proof also ran **at this exact head**: CI run 32372828676, job 96437124092, `head_sha 683782b1b41b15e4868cff6bc90e3f59f92d7c20`, conclusion `success`, with "Run writable DB proof against staging" and "Run the T1 live proof suites against staging" both succeeding. The harvested `runtime_proof` arrays below remain the ones from `29d05ebc` — that is their honest provenance — but the exact-head run independently corroborates the diff-binding argument rather than resting on it.
