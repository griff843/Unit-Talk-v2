# UTV2-1868 Diff Summary

Generated at: 2026-09-09T09:57:02.457Z
Issue: UTV2-1868
Tier: T2
Lane type: runtime
Branch: claude/utv2-1868-game-line-result-side
PR URL: https://github.com/griff843/Unit-Talk-v2/pull/1546
Head SHA: 2af53643c07b6e815a19c3443f4e754be02631be
Merge SHA: 0432f9d62cfd1487e43bdfe66903d8cb9813622c
Diff base: 2644d2742bd95b9c34c0bd2121162420163f9f46
Diff target: 2af53643c07b6e815a19c3443f4e754be02631be

## Git Diff Stat
```
.ops/sync/UTV2-1868.yml                    | 295 +++++++++++++++++++++++++++++
 apps/ingestor/src/ingestor.test.ts         | 220 +++++++++++++++++++++
 apps/ingestor/src/results-resolver.test.ts |   1 +
 apps/ingestor/src/results-resolver.ts      |  77 +++++++-
 apps/ingestor/src/sgo-fetcher.ts           | 109 ++++++++---
 docs/06_status/lanes/UTV2-1868.json        |  36 ++++
 6 files changed, 711 insertions(+), 27 deletions(-)
```

## Git Name Status
```
A	.ops/sync/UTV2-1868.yml
M	apps/ingestor/src/ingestor.test.ts
M	apps/ingestor/src/results-resolver.test.ts
M	apps/ingestor/src/results-resolver.ts
M	apps/ingestor/src/sgo-fetcher.ts
A	docs/06_status/lanes/UTV2-1868.json
```

## Manifest Files Changed
- No files_changed entries recorded.

## SHA Binding
Head SHA: 2af53643c07b6e815a19c3443f4e754be02631be
Merge SHA: 0432f9d62cfd1487e43bdfe66903d8cb9813622c
