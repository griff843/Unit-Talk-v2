...
unit suite                   5036 tests, 5036 pass, 0 fail
focused executor suites      81 tests, 81 pass, 0 fail
r-level check                PASS — 9 changed files, rules matched: (none)
mutation battery             8 of 8 defect classes DETECTED
...
| `pnpm test` | PASS | 5036 tests, 0 failures, within `pnpm verify:static`. |
| `pnpm exec tsx --test scripts/ops/{execution-packet,claude-exec,codex-exec}.test.ts` | PASS | 81 tests, 0 failures. |
| `pnpm exec tsx scripts/ci/r-level-check.ts --base origin/main --head HEAD` | PASS | 9 changed files. Rules matched: (none). |
