# UTV2-1861 Diff Summary

Generated at: 2026-09-11T00:31:20.881Z
Issue: UTV2-1861
Tier: T1
Lane type: modeling
Branch: claude/utv2-1861-admit-track-only-to-grading
PR URL: N/A
Head SHA: 2b4342515dbd86cd4ed32ac495308053eb5daebf
Merge SHA: ed336a1502874a375e66490891390d6efeeac07d
Diff base: 3ed5a940e130c5f9e79c67de47acc02bec9034eb
Diff target: 2b4342515dbd86cd4ed32ac495308053eb5daebf

## Git Diff Stat
```
.ops/sync/UTV2-1861.yml                 | 186 +++++++++++++++++++++++++
 apps/api/src/grading-service.test.ts    | 236 +++++++++++++++++++++++++++++++-
 apps/api/src/grading-service.ts         |  26 +++-
 apps/api/src/settlement-service.ts      |  41 +++++-
 docs/06_status/lanes/UTV2-1861.json     |  39 ++++++
 docs/06_status/proof/UTV2-1861/.gitkeep |   0
 6 files changed, 520 insertions(+), 8 deletions(-)
```

## Git Name Status
```
A	.ops/sync/UTV2-1861.yml
M	apps/api/src/grading-service.test.ts
M	apps/api/src/grading-service.ts
M	apps/api/src/settlement-service.ts
A	docs/06_status/lanes/UTV2-1861.json
A	docs/06_status/proof/UTV2-1861/.gitkeep
```

## Manifest Files Changed
- No files_changed entries recorded.

## SHA Binding
Head SHA: 2b4342515dbd86cd4ed32ac495308053eb5daebf
Merge SHA: ed336a1502874a375e66490891390d6efeeac07d
