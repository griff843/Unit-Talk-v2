...
- [x] Finalization refuses PR identity disagreement, atomically serializes merge-mutex acquisition plus its orphan-recoverable journal, preserves concurrent Linear labels with relation-specific mutations, and always reruns current-state reconciliation.

## EVIDENCE:

```text
Focused tests: 39 passed, 0 failed
Static verification: PASS, including 2,191 root tests
...
|---|---|---|
| `pnpm verify:static` | PASS | DB-client boundary, sync, system-alignment, automation-coverage, environment, lint, `pnpm type-check`, build, `pnpm test`, Smart Form verification, and command verification completed with exit 0; 2,191 root tests passed. |
| `pnpm exec tsx --test scripts/ops/lane-finalize.test.ts scripts/ops/lane-link-pr.test.ts` | PASS | 39 tests passed, 0 failed. |
...
| Partial-finalize recovery | PASS | Regression coverage atomically serializes cross-issue acquisition before the existing merge mutex, serializes the issue journal, cleans up refused acquisition journals, reclaims a dead local journal owner, resumes completed steps, and reruns reconciliation even when a prior journal recorded it. |
