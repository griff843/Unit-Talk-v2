...
| `apps/command-center/src/lib/data/analytics.test.ts` | +244 / -0 | **New.** 19 tests over pricing, cohort statistics, attribution and Track Only exclusion. |
...
Total: **8 files, +638 / -118**. No migrations, no workflow files, no `apps/api`, no contracts, no `scripts/`.
...
2. **No new odds conversion.** `pickProfitUnits` goes through `americanToDecimal` /
   ...
   implementation of a money calculation is a second thing that can be wrong.
