...
UTV2-492 (P7A-02: gate the real queueing path for awaiting_approval picks, PR #222, commit `144d0de`) Done. Primary brake in `apps/api/src/controllers/submit-pick-controller.ts`: autonomous non-human sources (`system-pick-scanner`, `alert-agent`, `model-driven`) transition `validated → awaiting_approval` at submission and do NOT auto-enqueue for distribution. Defense-in-depth guards in `run-audit-service.enqueueDistributionWithRunTracking` (DB re-fetch + `distribution.enqueue.blocked` audit) and `distribution-service.enqueueDistributionWork` (`AwaitingApprovalBrakeError`). `GOVERNANCE_BRAKE_SOURCES` authoritative in `distribution-service.ts`. `board-construction` explicitly excluded — operator-triggered, not autonomous (PM repo-truth correction 2026-04-10). Human-relayed sources (`smart-form`, `api`, `discord-bot`, `feed`, `system`) retain existing queueing behavior. 11 new tests, all passing.
UTV2-491 (P7A-01: add awaiting_approval lifecycle state, commit `0c95b1e`) Done — FSM extended, review controller extended via UTV2-509, operator-web surfaces via UTV2-510, command-center surfaces via UTV2-511. Phase 7A remaining: UTV2-493 (approval-path enqueue bridge) + UTV2-494 (proof bundle).

2026-04-10 — **Phase 7 OPENED — governance-first charter. Ratification gate in progress (UTV2-485).**
Phase 7 redirects the next build from "feedback loop + UX hardening" to **governance-first**: `awaiting_approval` lifecycle state → ingress unification (retire `system-pick-scanner` + `alert-agent` direct submission) → closed feedback loop → real model layer (replace `computeModelBlend(devig, devig, 0, 0)` + constant `uncertainty=0.2`) → calibration + schema cleanup. Linear project "Phase 7 - Governed Syndicate Machine" (UTV2-485..508, 24 issues, 5 sub-phases 7A–7E). Draft charter: `docs/06_status/PHASE7_PLAN_DRAFT.md`. Ratification bundle: `docs/06_status/PHASE7R_RATIFICATION.md`. Current runtime state recorded as facts: `SYSTEM_PICK_SCANNER_ENABLED=false` (default), `SYNDICATE_MACHINE_ENABLED=false` (default — governed Phase 2–6 pipeline dormant in prod), `ALERT_AGENT_ENABLED=true` (default — **only live autonomous producer today**). UTV2-485 Ready; all other 7A–7E issues Backlog pending gate.
...
**Launch positioning:** Pick operations + distribution + tracking platform with real market edge computation. Sprint D (Intelligence v1) is complete. Locked roadmap Phases 1–6 closed 2026-04-10 (PROGRAM_STATUS line 10). NOT a syndicate-level intelligence system — that requires **Phase 7 (governance-first)**: ratification gate → governance brake (awaiting_approval lifecycle state) → ingress unification (retire direct non-human producers) → closed feedback loop → real model layer → calibration + schema cleanup. Charter: Linear project "Phase 7 - Governed Syndicate Machine" (UTV2-485..508). Draft reference doc: `docs/06_status/PHASE7_PLAN_DRAFT.md`. Ratification bundle: `docs/06_status/PHASE7R_RATIFICATION.md` (UTV2-485).
