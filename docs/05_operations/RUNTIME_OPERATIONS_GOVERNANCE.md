...
|---|---|---|---|
| Orchestrator / break-glass invoker | Claude, current session | Declare incidents; restart a single wedged service with PM sign-off (see §4 — pending `INCIDENT_RUNBOOK.md` ratification); invoke break-glass (rollback/pause/restore only) when Griff is unreachable and all four break-glass conditions hold | Declare/break-glass: `BREAK_GLASS_PROTOCOL.md` §1–2. Restart: PM-gated per §4, not independently ratified |
...
|---|---|---|---|
| **Restart** a single wedged service (e.g. ingestor container) | Claude, with Griff sign-off | **PM-gated.** `INCIDENT_RUNBOOK.md` describes this as normal first response, but that document is itself still marked DRAFT — PM ratification required before treated as binding process (UTV2-1428). This chapter cannot grant standing no-PM-gate authority by citing an unratified source. Restart requires a quick PM go-ahead until `INCIDENT_RUNBOOK.md` ratifies; this row should be revisited (and the gate potentially lifted) once it does. Automated supervisor auto-restart (`INGESTOR_RUNTIME_SUPERVISION.md`) is unaffected by this row — that is existing, already-ratified automated behavior, not a human/Claude action. | No independently-ratified source grants this; gated pending UTV2-1428 |
| **Restart** the full compose stack / multiple services | Claude, with Griff sign-off | Same PM gate as the row above, plus: do not restart the whole stack for a single-service symptom | Same as the row above |
