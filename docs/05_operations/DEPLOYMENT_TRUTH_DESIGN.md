...
`probeDeploySha`'s own evidence-of-interest is `stage: "promote"` (and `stage: "rollback"` when it supersedes a promote). Canary-stage records are never considered by `deploy_sha_alignment` — they exist for `deploy.yml`'s own internal use (canary health-gating before promote runs) and are out of scope for readiness. This distinction must be explicit in the implementation, not incidental.
...
§12b closes this: the restricted host reader (same hardened SSH boundary as §12a) additionally inspects the **actual running Compose project's services and their immutable image IDs/digests** (`docker inspect` / `docker compose ps --format json`, not `.unit-talk-release`) and compares that against the confirmed-tag record. Disagreement is `unreadable`, with the specific mismatch named in evidence text (e.g. "receipt claims tag X; container `api` is actually running digest Y").
...
| 17 | (Phase 2, needs §12b) §12b's live daemon inspection disagrees with the artifact/§12a-derived evidence | `unreadable`, explicit disagreement (claimed tag vs. actually-running digest) noted in evidence text |
...
1. **Phase 1 (required together, not separable):** §3.5 (operation identity/stage) + §4/§5/§9 (host-side intent + confirmation, forward and rollback) + §5.5 (durable journal protocol) + §12a (intent/confirmation host read, with its forced-command/no-PTY SSH hardening) + §13 (fail-closed three-way classification) + §8/§10. §12a is part of the *core correctness fix*, not an optional enhancement to it — regression matrix rows 5–8, 11, and 13–15 all depend on it existing. This is the direct consequence of Codex's finding that §13's "no evidence" branch is unsound without a host-read capability.
2. **Phase 2:** §12b (live container/daemon reconciliation against actual running state, not just receipt files) — a strictly larger capability than §12a (broader host inspection, not just reading two small JSON files), may ship as a separate, later lane. Regression matrix rows 16–17 are phase-2-only and must not be silently claimed as covered by a phase-1 implementation.
