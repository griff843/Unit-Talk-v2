...
The two ratified evidence forms above are authoritative. For continuity and auditability, the role **should** capture material read-only diagnostics that inform an incident decision in the applicable incident declaration artifact or T1 evidence bundle. This is operating guidance, not a third evidence form, a new mandatory logging obligation, or authority to substitute a Linear comment for either ratified artifact. Chat narrative and Linear comments may coordinate work, but neither independently satisfies the evidence standard. A useful diagnostic record names what was done, why, the exact commands or queries run with timestamps, and the observed output.

**Restart actions should capture pre/post state**: before the restart, the evidence of the wedge (last logged cycle timestamp, staleness measurement, relevant log excerpt); after the restart, the evidence of recovery (a new completed cycle, fresh `pnpm ops:brief` output) — following the confirm-after-restart guidance in `INCIDENT_RUNBOOK.md` §1. Because that runbook remains DRAFT pending PM ratification, missing pre/post capture makes the restart evidence-incomplete; this charter does not independently make the action unauthorized or create a new authorization gate. The restart still requires the authority gate named in `RUNTIME_OPERATIONS_GOVERNANCE.md` §4.
