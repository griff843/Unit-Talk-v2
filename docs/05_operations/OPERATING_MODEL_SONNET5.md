...
- **Codex:** implements scoped code lanes, constrained by the existing T1/Tier C rules in `three-brain.md` (Rule 1, Rule 2 remain absolute — Codex never touches `packages/domain/`, `packages/contracts/`, migrations, lifecycle, or auth, regardless of tier).
