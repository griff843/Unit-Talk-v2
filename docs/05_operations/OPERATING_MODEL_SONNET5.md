...
- **Fable 5 (UTV2-1569 — bounded, mechanically-enforced pilot, NOT active by default):** following the pilot-evaluation issue's **PILOT LONGER — lean YES, narrow scope** conclusion, Fable 5 is a valid `model` value again, but only as an advisory reviewer for four ratified trigger classes (repeated architecture bounce, live-state root-cause, product synthesis without precedent, Build Mode certification review — full list in `.claude/commands/three-brain.md`'s "Fable 5 pilot routing" section), and only for the duration of a bounded pilot: 8 qualifying tasks, 30 calendar days, or a fixed usage ceiling, whichever comes first.

  **This is mechanically enforced, not merely documented** — the gap the earlier pilot attempt (PR #1287, unmerged) left open ("the 8-task/30-day/usage-budget limits above are tracked manually... not yet mechanically validated or count-capped in code"). `scripts/ops/planning-model-routing.ts#resolveAndRecordPlanningModel()` is the sole function `ops:lane-start` calls at lane-creation time (Phase 3 of `/dispatch`, before the Phase 4 planning subagent ever spawns) for the planning path, and `resolveAndRecordFableAdvisoryReview()` is its Phase-5 advisory-review counterpart; both resolve AND, on a genuine Fable selection, atomically record the qualifying task/usage against pilot state in the same operation. Both fail closed to Sonnet whenever the policy is disabled, the trigger class is unknown/skip-listed/individually-disabled, activation/expiry dates are missing/malformed/inconsistent, or `docs/05_operations/FABLE_PILOT_STATE.json` reports anything other than "active and within all caps" (`scripts/ops/fable-pilot-state.ts`). Work that does not match one of the four ratified classes — novel architecture, constitutional scope, unresolved ambiguity outside those classes — still escalates to Griff per Rule 9, exactly as before.

  **Current status: pending, not activated.** `FABLE_PILOT_STATE.json`'s `status` is `"pending"` — the pilot's clock has not started. Every `resolveAndRecordPlanningModel()`/`resolveAndRecordFableAdvisoryReview()` call currently falls back to Sonnet regardless of trigger class, because activating the pilot (setting `status` to `"active"` and a real `activated_at`/`expires_at`) is a real operational decision reserved for Griff, not something this mechanism's own landing does automatically.

  **Advisory only, always:** never a merge authority, never a `pm-verdict/v1` substitute, never a vote in any T1-M quorum, and never touches Rule 9 or Griff's T1-H authority — this holds for every trigger class, including certification review.

  **Suspension/kill switch, independent of the model being evaluated:** `FABLE_PILOT_STATE.json`'s `status` can be set to `"suspended"` at any time for any operational reason, independent of `docs/05_operations/policies/fable-pilot-policy.json`'s separate `pilot_enabled` flag — either one being off blocks all Fable routing.

  **Rollback:** `docs/05_operations/FABLE_PILOT_ROLLBACK.md` — a mechanical function call (`scripts/ops/fable-pilot-rollback.ts`) that makes Fable permanently unselectable, plus a documented git-revert of the prose/allowlist diffs this pilot introduced (this document, `three-brain.md`, `agent-role-contracts.md`, `docs/governance/AGENT_SKILL_CONTRACTS.md`, `scripts/ops/contract-validator.ts`). Proven, not asserted: `scripts/ops/fable-pilot-rollback.test.ts` runs the rollback against a fully-eligible pilot fixture and asserts Fable becomes unselectable for all four trigger classes.

  **Closeout:** at pilot expiry (either cap reached), `docs/05_operations/FABLE_PILOT_CLOSEOUT_TEMPLATE.md` is filled in for a fresh `FABLE 5 PERMANENT INTEGRATION: YES / NO / EXTEND` decision packet. No permanent routing or authority change happens without a later, exact-head Griff decision — this document's own governance-change protocol (§7) applies to that decision the same as any other.
