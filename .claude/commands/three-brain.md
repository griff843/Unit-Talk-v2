...
| T1, repeated-bounce (2+ CHANGES_REQUIRED on the same architectural question), product-synthesis with no existing precedent, or hard root-cause/mechanistic-verification work | `fable` (Fable 5) — **advisory-only, PILOT (UTV2-1568), not permanent** — narrow, per `OPERATING_MODEL_SONNET5.md` §1 | Pilot capped at 8 qualifying real tasks or 30 days, whichever first; Fable's output is a review input only — it is never a merge authority, never a `pm-verdict/v1` substitute, and never counts as a vote in any T1-M quorum. Ambiguous-boundary/scope-ambiguity T1 work outside this narrow class remains a Rule 9 Griff-escalation trigger, exactly as UTV2-1390 set it — the pilot does not absorb the general case, and reverts to Rule-9-only automatically once its cap is hit. |
| T2 / T3 Claude | *(none — no planning subagent)* | Bounded work; orchestrator session handles directly |

**Default:** `sonnet`. Fable 5 is routed to only for the narrow repeated-bounce / product-synthesis / hard-root-cause class above, only for the duration of the UTV2-1568 pilot, and only in an advisory capacity — everything else that would have escalated under the UTV2-1390 model still does: genuinely novel-architecture, constitutional-scope, or ambiguous-boundary T1 work outside that narrow class is a Rule 9 Griff-escalation trigger (scope ambiguity / Tier C), not a model-routing decision. Any change to this table, this document, or the T1-M/T1-H classifier (UTV2-1555, not yet built — this clause is protective in advance of its build) is itself always Rule 9 regardless of what Fable's review of such a change concludes (`OPERATING_MODEL_SONNET5.md` §1). Full model policy: `docs/05_operations/OPERATING_MODEL_SONNET5.md`.
