...
const manifest = (required, issue_id = work) => ({ issue_id, p0_protocol: {
...
test('new explicit false needs review while committed false is trusted; candidate true escalates', () => {
  assert.equal(classifyP0Evidence({issueId: work, candidateManifest: manifest(false)}).classification, 'unknown');
...
test('actual workflow evaluator blocks no approval, erased base P0 and missing/failed artifacts', async () => {
  ...
  await assert.rejects(evaluatePullRequest(fakeGitHub({required: false, approval: false})), /unknown/);
