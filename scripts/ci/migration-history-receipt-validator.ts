...
if (failures.length > 0) {
  console.error(`migration-history-receipt-validator: FAIL (${failures.length})`);
  for (const f of failures) console.error(`  - ${f}`);
  process.exit(1);
}
console.log(`migration-history-receipt-validator: PASS (${manifest.receipts.length} receipts)`);
