...
const fingerprint = (): string => psql(['-tA', '-c', FINGERPRINT_SQL]).trim();
...
if (afterBaseline === '') {
  console.error('FAIL: baseline produced an empty schema — the replay root did not apply at all');
  process.exit(1);
}
console.log(`phase 1: baseline applied, ${afterBaseline.split('\n').length} catalog entries`);
...
if (afterReceipts !== afterBaseline) {
  console.error('migration-history-replay-drill: FAIL — receipts mutated the schema');
  const before = new Set(afterBaseline.split('\n'));
  const after = new Set(afterReceipts.split('\n'));
  for (const e of after) if (!before.has(e)) console.error(`  + ${e}`);
  for (const e of before) if (!after.has(e)) console.error(`  - ${e}`);
  process.exit(1);
}
console.log('phase 3: schema fingerprint byte-identical before and after receipts');
