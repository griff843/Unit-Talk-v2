# Package: @unit-talk/events

Shared domain event type definition.

## Role in Unit Talk V2

- System layer: **contract (event shape)**
- Pure: yes
...
**Depended on by:** `apps/api`, `apps/worker`, `apps/discord-bot`

## What Lives Here

- `src/index.ts` — `DomainEvent<TPayload>` interface: `{ name: string; payload: TPayload; occurredAt: string }`
...
- Keep as a pure type contract
- Do not add event bus, handlers, or dispatch logic here

## What NOT to Do

- Do not add runtime event infrastructure (that belongs in apps)
